...
class Discriminator(nn.Module):
    def __init__(self, dim, cfg: Wav2vec_UConfig):
        ...
        # Write a nn.Sequential block of (*emb_net, dropout of discriminator_dropout, and *inner_net)
...
class Wav2vec_U(nn.Module):
    ...
    def __init__(self, target_dict, cfg: Wav2vec_UConfig=Wav2vec_UConfig):
        ...
        # Call the Discriminator class with output_size and config
        self.discriminator = Discriminator(output_size, cfg)

        self.pca_A = self.pca_b = None
        d = cfg.input_dim
        # Call the JoinSegmenter class with the segmentation from config
        self.segmenter = JoinSegmenter(cfg.segmentation)
        # Call the Generator class with d, output_size and config
    ...
    def normalize(self, dense_x):
        ...
        hard_x = (
         ...
         # Call torch.mean on hard_x after converting it to float and use dim == 0 
    ...
    def forward(
        ...
        if d_step:
            ...
            if self.training and self.gradient_penalty > 0:
                # Call the calc_gradient_penalty function you implemented with token_x and dense_x
            ...
            if self.smoothness_weight > 0:
                # TODO: implement smoothness loss
