...
class UnpairedAudioTextConfig(FairseqDataclass):
    ...
    data: str = "/content/data/11-685-s23-hw5/train_dev_data/audio_feats"
    text_data: str = "/content/data/11-685-s23-hw5/train_dev_data/text_data"
...
class UnpairedAudioText(FairseqTask):
    ...
    def __init__(
        ...
        if cfg.kenlm_path is not None:
            import kenlm
             # Call the kenlm model, just with the kenlm path from config
            self.kenlm = kenlm.Model(cfg.kenlm_path)

        self.word_kenlm = None
        if cfg.word_kenlm_path is not None:
            import kenlm
            # Call the kenlm model, but with the word kenlm path from config
        ...
        if cfg.word_decoder_config is not None:
            # Call the Kaldi Decoder with the word decoder config and beam == 10
            self.kaldi_decoder = KaldiDecoder(cfg.word_decoder_config, beam=10) 

            def compute_word_score(logits, padding):
                 # Call decode of kaldi_decoder that you defined above, with arguments as logits and padding
    ...
    def valid_step(self, sample, model):
        ...
        for i, (x, t, id) in enumerate(
            ...
            if self.kenlm is not None:
                ...
                # Call compute_lm_score on the pred_str
